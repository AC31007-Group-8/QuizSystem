...
/**
 * @author Erik J (Adapted from StudentQuizManager by Can G, Allan M)
 */
public class QuizViewer {
    ...
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(QuizViewer.class);
    
    @Init
    public static void init() {
        get("/staff/viewQuiz", QuizViewer::serveQuiz);
        post("/staff/viewQuiz", "application/json", QuizViewer::receiveQuiz);
    }

    
    //SERVE QUIZ
    
    public static Object serveQuiz(Request req, Response res){
        ...
        //Check if the user viewing is the creator if it's unpublished
        ...
        map.put("publishStatus", quiz.isPublish_status());
        
        Integer inMinutes = quiz.getTime_limit();
        if (inMinutes == null){
             map.put("timeLimit", "None");
        }
        else{
            int h = inMinutes/60;
        ...
        if (quiz.isPublish_status()){
            ...
            map.put("publishStatus", "False");
        }
            
      
        map.put("allQuestions", allQuestions);

        return eng.render(eng.modelAndView(map, "viewQuiz.mustache"));
    ...
    public static Object receiveQuiz(Request req, Response res){
       ...
       private static void sendEmail(String pageHtml, String message) throws FileNotFoundException, AddressException, MessagingException{
        ...
        String path = QuizViewer.class.getResource("/temp").getPath();
